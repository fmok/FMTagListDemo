...
{
    Class tagClass = _tagClass?_tagClass : [FMTagButton class];
    
    // 创建标签按钮
    FMTagButton *tagButton = [tagClass buttonWithType:UIButtonTypeCustom];
    if (_tagClass == nil) {
...
- (void)clickTag:(UIButton *)button
{
    ...
    if (_clickTagBlock) {
        _clickTagBlock(button.currentTitle, button.tag);
    }
}
...
{
    // 获取对应的标题按钮
    FMTagButton *button = self.tags[tagStr];
...
- (void)updateTagButtonFrame:(NSInteger)i extreMargin:(BOOL)extreMargin
{
    ...
    // 获取当前按钮
    FMTagButton *tagButton = self.tagButtons[i];
    // 判断是否设置标签的尺寸
...
- (void)setupTagButtonCustomFrame:(UIButton *)tagButton preButton:(UIButton *)preButton extreMargin:(BOOL)extreMargin
{
    ...
    // 宽度超出，赋值最大宽度，末尾截字，省略号
    if (self.bounds.size.width <= btnW + 2*_tagMargin) {
        btnW = self.bounds.size.width - 2*_tagMargin;
    }
